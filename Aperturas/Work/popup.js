...
const bindInventoryTool = () => {
  const textarea = $('inv-textarea-popup');
  const btnExecute = $('inv-btn-execute-popup');
  const btnClear = $('inv-btn-clear-popup');
  const btnCopy = $('inv-btn-copy-popup');
  const btnExport = $('inv-btn-export-popup');
  const btnExample = $('inv-btn-example-popup');
  const btnCopyTemplate = $('inv-btn-copy-template-popup');
  const output = $('inv-output-popup');

  const statWos = $('inv-wos');
  const statSeus = $('inv-seus');
  const statHosts = $('inv-hosts');
  const statDdis = $('inv-ddis');

  if (!btnExecute) return;

  let parsedDictionaries = [];

  const updateLiveStats = (text = '') => {
    const woMatches = (text.match(/WO:?/gi) || []).length;
    const seuMatches = (text.match(/CODI SEU:?\s*\$?([A-Z0-9\._]+)\$?|\$SEU[^\$\s]+\$/gi) || []).length;
    const hostMatches = (text.match(/Hostname:?\s*([A-Za-z0-9_\-]+)/gi) || []).length;
    const ddiMatches = (text.match(/DDI:?\s*([0-9]{8,15})/gi) || []).length;

    if (statWos) statWos.textContent = woMatches;
    if (statSeus) statSeus.textContent = seuMatches;
    if (statHosts) statHosts.textContent = hostMatches;
    if (statDdis) statDdis.textContent = ddiMatches;
  };
  ...
  const buildRemedyAridMapping = (dict) => {
    // Extract 9-digit DDI number
    const ddiMatch = (dict.ddi || dict.rawText || '').match(/\b\d{9}\b/);
    const ddiNum = ddiMatch ? ddiMatch[0] : (dict.ddi || '').replace(/\D/g, '');
    ...
    return {
      ...
      "arid_WIN_5_1000000001": dict.company || '',
      "arid_WIN_5_2000003720": dict.organization || '',
      "arid_WIN_5_200000006": dict.departament || '',
      "arid_WIN_5_2000002850": "Deployed",
      "arid_WIN_5_260700001": formattedDate,
      "arid_WIN_7_8": dict.hostname || ''
    };
  };

  const fillAndSaveActiveTab = (dictMap, callback) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      const executeFill = () => {
        if (chrome.scripting) {
          chrome.scripting.executeScript({
            ...
            func: (mapData) => {
              const findRemedyFieldInline = (idOrSfx) => {
                if (!idOrSfx) return null;
                const sfx = idOrSfx.includes('_') ? idOrSfx.split('_').pop() : idOrSfx;
                const isVisible = el => el && (el.offsetWidth > 0 || el.offsetHeight > 0 || el.getClientRects().length > 0);

                const candidates = Array.from(document.querySelectorAll(`[id$="_${sfx}"], [id*="_${sfx}"], [id="${idOrSfx}"]`));
              ...
              Object.keys(mapData).forEach(id => {
                ...
                if (!val) return;
                const el = findRemedyFieldInline(id);
                if (el) {
                  el.focus();
                  if (el.tagName === 'SELECT') {
                    ...
                    for (let i = 0; i < el.options.length; i++) {
                      if (el.options[i].value === val || el.options[i].text.trim().toLowerCase() === valLower) {
                        el.selectedIndex = i;
                        matched = true;
                        break;
                      }
                  ...
                  ['focus', 'keydown', 'keypress', 'keyup', 'input', 'change', 'blur'].forEach(ev => {
      ...
      chrome.tabs.sendMessage(tabId, {
        action: 'FILL_ARCONTE_FORM',
        data: dictMap,
        autoSave: false
      }, (res) => {
        // Read lastError explicitly to mark it handled and prevent Chrome extension warning
        const _ignoreErr = chrome.runtime.lastError;
        executeFill();
  ...
  const processWoText = (text) => {
    if (!text) {
      if (output) output.innerHTML = `<div class="text-center text-warning p-2 small">No se encontró texto de plantilla WO en el portapapeles.</div>`;
      return;
    }
    ...
    rawBlocks.forEach((block, idx) => {
      ...
      let woCode = getFieldValue(trimmed, ['WO']);
      if (!woCode) {
        const firstLine = trimmed.split('\n')[0];
        if (firstLine.toUpperCase().startsWith('WO:')) {
          woCode = firstLine.substring(3).trim();
        }
      }

      const dict = {
        wo: woCode || `WO_${idx + 1}`,
        tipo: getFieldValue(trimmed, ['TIPO', 'TIPO DE ACCION']),
        eu: getFieldValue(trimmed, ['EU']),
        company: getFieldValue(trimmed, ['COMPANY']),
        organization: getFieldValue(trimmed, ['ORGANIZATION']),
        departament: getFieldValue(trimmed, ['DEPARTAMENT']),
        codiSeu: getFieldValue(trimmed, ['CODI SEU', 'CODI_SEU']),
        nombreSala: getFieldValue(trimmed, ['Nombre de sala', 'SALA']),
        hostname: getFieldValue(trimmed, ['Hostname', 'HOST']),
        ddi: getFieldValue(trimmed, ['DDI']),
        plataforma: getFieldValue(trimmed, ['plataforma', 'PLATAFORMA']),
        rawText: trimmed
      };

      dict.remedyAridMap = buildRemedyAridMapping(dict);
    ...
    if (parsedDictionaries.length > 0) {
      fillAndSaveActiveTab(parsedDictionaries[0].remedyAridMap);
    }
  };
  ...
  btnExecute.addEventListener('click', async () => {
    try {
      ...
      if (output) output.innerHTML = `<div class="text-center text-danger p-2 small">Permiso denegado al leer el portapapeles. Copia la plantilla y pulsa CTRL+A en Remedy.</div>`;
    }
  });

  if (btnExample) {
    btnExample.onclick = () => {
      const sampleText = `WO: WO000000XXXXXXX\nTIPO: ALTA ARCONTE\nEU: NOMBRE SEU\nCOMPANY: DEPARTAMENT DE JUSTÍCIA\nORGANIZATION: ADMINISTRACIÓ DE JUSTÍCIA - TARRAGONA\nDEPARTAMENT: ADMINISTRACIÓ DE JUSTÍCIA - TARRAGONA\nCODI SEU: $SEU.005563$\nNombre de sala: SALA DE VISTES 09 PLANTA 1 %ESP.002989.1%\nHostname: VNDSM02VX\nDDI: 933263128\nplataforma = %PLATAFORMA VIDEOCONFERENCIA CORPORATIVA%`;
      processWoText(sampleText);
    };
  }

  const renderPopupOutput = () => {
    if (!output) return;
    if (parsedDictionaries.length === 0) {
      output.innerHTML = `<div class="text-center text-muted py-3 small">No se detectaron plantillas de WO válidas.</div>`;
      return;
    }

    let html = `<div style="display:flex; flex-direction:column; gap:8px;">`;
    parsedDictionaries.forEach((dict, idx) => {
      let badgeBg = 'rgba(245, 175, 78, 0.15)';
      let badgeColor = '#f5af4e';
      if (dict.tipo.includes('ALTA')) { badgeBg = 'rgba(136, 212, 166, 0.15)'; badgeColor = '#88d4a6'; }
      else if (dict.tipo.includes('BAJA')) { badgeBg = 'rgba(234, 133, 117, 0.15)'; badgeColor = '#ea8575'; }
      else if (dict.tipo.includes('TRASLADO')) { badgeBg = 'rgba(124, 213, 224, 0.15)'; badgeColor = '#7cd5e0'; }

      html += `
        <div style="background: rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); padding:8px; border-radius:8px; font-size:10.5px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px; border-bottom:1px solid rgba(255,255,255,0.06); padding-bottom:4px; flex-wrap:wrap; gap:4px;">
            <span style="background:${badgeBg}; color:${badgeColor}; border:1px solid ${badgeColor}; font-weight:700; font-size:10px; padding:1px 6px; border-radius:4px;">
              ${escapeHtml(dict.wo)}: ${escapeHtml(dict.tipo || 'WO DICCIONARIO')}
            </span>
            <div style="display:flex; gap:3px;">
              <button class="btn btn-sm py-0 px-2 fill-save-crm-btn" data-dictidx="${idx}" style="font-size:9.5px; background:linear-gradient(135deg, #f5af4e 0%, #e09632 100%) !important; color:#141210 !important; font-weight:800; border:none !important; height:22px;">
                <i class="ph ph-magic-wand me-1"></i>Rellenar CRM
              </button>
              <button class="btn btn-sm py-0 px-1 copy-arid-btn" data-dictidx="${idx}" style="font-size:9px; background:rgba(136, 212, 166, 0.15); border:1px solid rgba(136, 212, 166, 0.3); height:22px; color:#88d4a6;" title="Copiar ARID Map">
                <i class="ph ph-list-checks"></i>
              </button>
              <button class="btn btn-sm py-0 px-1 copy-json-btn" data-dictidx="${idx}" style="font-size:9px; background:rgba(245, 175, 78, 0.15); border:1px solid rgba(245, 175, 78, 0.3); height:22px; color:#f5af4e;" title="Copiar JSON">
          ...
          <!-- Dictionary Grid -->
          <div style="font-family: Consolas, monospace; background: rgba(0,0,0,0.3); padding:6px; border-radius:6px; font-size:10px; margin-bottom:6px;">
            <div style="color:#88d4a6; font-weight:700; margin-bottom:2px;">{ /* Diccionario traducido */</div>
            <div style="padding-left:8px; display:flex; flex-direction:column; gap:1px;">
              <div><span style="color:var(--accent-amber);">"wo"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.wo)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"tipo"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.tipo)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"eu"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.eu)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"company"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.company)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"organization"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.organization)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"departament"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.departament)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"codiSeu"</span>: <span style="color:#88d4a6;">"${escapeHtml(dict.codiSeu)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"nombreSala"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.nombreSala)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"hostname"</span>: <span style="color:#cbb2e0;">"${escapeHtml(dict.hostname)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"ddi"</span>: <span style="color:#ea8575;">"${escapeHtml(dict.ddi)}"</span>,</div>
              <div><span style="color:var(--accent-amber);">"plataforma"</span>: <span style="color:#f5ece0;">"${escapeHtml(dict.plataforma)}"</span></div>
            </div>
            <div style="color:#88d4a6; font-weight:700; margin-top:2px;">}</div>
          </div>

          <!-- Mapeo Formulario ARID -->
          <div style="font-family: Consolas, monospace; background: rgba(136, 212, 166, 0.05); border:1px solid rgba(136, 212, 166, 0.2); padding:6px; border-radius:6px; font-size:9.5px;">
            <div style="color:#88d4a6; font-weight:700; margin-bottom:2px;"><i class="ph ph-magic-wand me-1"></i>Campos Remedy (arid_WIN_5_...):</div>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:2px; color:var(--text-muted);">
              <div>arid_WIN_5_200000020: <b style="color:#88d4a6;">${escapeHtml(dict.remedyAridMap['arid_WIN_5_200000020'])}</b></div>
              <div>arid_WIN_5_210000000: <b style="color:#88d4a6;">${escapeHtml(dict.remedyAridMap['arid_WIN_5_210000000'])}</b></div>
              <div>arid_WIN_5_2000002882: <b style="color:#ea8575;">${escapeHtml(dict.remedyAridMap['arid_WIN_5_2000002882'])}</b></div>
              <div>arid_WIN_5_260700001: <b style="color:#f5af4e;">${escapeHtml(dict.remedyAridMap['arid_WIN_5_260700001'])}</b></div>
            </div>
          </div>
        </div>
      `;
    ...
    output.querySelectorAll('.fill-save-crm-btn').forEach(btn => {
      btn.onclick = () => {
        const idx = parseInt(btn.dataset.dictidx);
        const item = parsedDictionaries[idx];
        if (item && item.remedyAridMap) {
          fillAndSaveActiveTab(item.remedyAridMap);
          flashBtn(btn, '<i class="ph ph-check me-1"></i>¡Rellenado!', '<i class="ph ph-magic-wand me-1"></i>Rellenar CRM');
        }
      };
    });

    output.querySelectorAll('.copy-json-btn').forEach(btn => {
      btn.onclick = () => {
        const idx = parseInt(btn.dataset.dictidx);
        const item = parsedDictionaries[idx];
        if (item) {
          const cleanObj = { ...item };
          delete cleanObj.rawText;
          delete cleanObj.remedyAridMap;
    ...
    output.querySelectorAll('.copy-arid-btn').forEach(btn => {
      btn.onclick = () => {
        const idx = parseInt(btn.dataset.dictidx);
        const item = parsedDictionaries[idx];
        if (item && item.remedyAridMap) {
          navigator.clipboard.writeText(JSON.stringify(item.remedyAridMap, null, 2));
          flashEl(btn);
        }
      };
    });
...
plataforma = %PLATAFORMA VIDEOCONFERENCIA CORPORATIVA%`;
  ...
  if (btnCopyTemplate) {
    btnCopyTemplate.addEventListener('click', () => {
      navigator.clipboard.writeText(PLANTILLA_WO).then(() => {
        const orig = btnCopyTemplate.innerHTML;
        btnCopyTemplate.innerHTML = '<i class="ph ph-check me-1"></i>¡Copiada!';
        btnCopyTemplate.style.background = 'rgba(52,211,153,0.35)';
        setTimeout(() => {
          btnCopyTemplate.innerHTML = orig;
          btnCopyTemplate.style.background = 'rgba(52,211,153,0.15)';
        }, 1500);
      }).catch(() => {
        // Fallback
        const ta = document.createElement('textarea');
        ta.value = PLANTILLA_WO;
...
plataforma = %PLATAFORMA VIDEOCONFERENCIA CORPORATIVA%`;
  ...
  if (btnClear) {
    btnClear.addEventListener('click', () => {
      ...
      parsedDictionaries = [];
      updateLiveStats();
      if (output) output.innerHTML = `<div class="text-center text-muted py-3 small"><i class="ph ph-file-text d-block fs-4 mb-1" style="opacity:0.5;"></i>Pega la plantilla de WO arriba y pulsa EJECUTAR PROCESAMIENTO</div>`;
    });
  }

  if (btnCopy) {
    btnCopy.addEventListener('click', () => {
      if (parsedDictionaries.length === 0) return;
      const exportList = parsedDictionaries.map(d => {
        const c = { ...d };
        delete c.rawText;
        return c;
      });
      const text = JSON.stringify(exportList.length === 1 ? exportList[0] : exportList, null, 2);
      navigator.clipboard.writeText(text);
      flashBtn(btnCopy, '<i class="ph ph-check me-1"></i>¡Copiado!', '<i class="ph ph-copy me-1"></i>Copiar Diccionario');
    });
  }
  ...
  if (btnExport) {
    btnExport.addEventListener('click', () => {
      if (parsedDictionaries.length === 0) return;
      let csv = 'WO,Tipo,EU,Company,Organization,Departament,CodiSEU,NombreSala,Hostname,DDI,Plataforma\n';
      parsedDictionaries.forEach(d => {
        csv += `"${d.wo}","${d.tipo}","${d.eu}","${d.company}","${d.organization}","${d.departament}","${d.codiSeu}","${d.nombreSala.replace(/"/g, '""')}","${d.hostname}","${d.ddi}","${d.plataforma.replace(/"/g, '""')}"\n`;
      });
      const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `wo_diccionario_${Date.now()}.csv`;
      link.click();
    });
  }
};
